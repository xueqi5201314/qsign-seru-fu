...
class FEBound {
    private val mConfigEnCode = arrayOf(
    ...
    private val mConfigDeCode = arrayOf(
    ...
    fun initAssertConfig(baseDir: File?) {
        ...
        val `object` = JSON.parseObject(text)
        val en = `object`.getJSONArray("en")
        for (i in en.indices) {
            val keys = en.getJSONArray(i)
            for (j in keys.indices) {
                mConfigEnCode[i][j] = keys.getByteValue(j)
            }
        }
        val de = `object`.getJSONArray("de")
        for (i in de.indices) {
            val keys = de.getJSONArray(i)
            for (j in keys.indices) {
    ...
    fun checkCurrent(): Int {
        var tmp = 0
        for (bytes in mConfigEnCode) {
            for (aByte in bytes) {
                tmp += aByte.toInt()
            }
        }
        for (bytes in mConfigDeCode) {
            for (aByte in bytes) {
                tmp += aByte.toInt()
            }
        }
